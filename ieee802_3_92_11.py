...
class MatedTestFixture:
	...
	def plotAll(self):
		...
		matplotlib.pyplot.text(1,-0.5,r"$FOM_{ILD} = $" + "{:0.3f} dB".format(self.FOM_ILD), fontsize=16, bbox = dict(facecolor='white', alpha=0.5))
		...
		matplotlib.pyplot.text(1,-30,r"$\sigma_{nx} = $" + "{:0.3f} mV".format(self.sigma_nx*1e3) +
			'\n' + r"$\sigma_{fx} = $" + "{:0.3f} mV".format(self.sigma_fx*1e3) +
			'\n' + r"$\sigma_{x} = $" + "{:0.3f} mV".format(self.sigma_x*1e3),
